<div>
    ...
    <div class="relative">
        <div class="flex justify-start items-center mb-1">
            ...
            <div class="flex-grow h-10 flex items-center">
                <button
                    class="px-3 py-2 border border-green-400 rounded-md bg-white 
            ...
            <div class="flex items-center space-x-1">
                ...
                <div x-data="{ init() {
                    ...
                    <button wire:click="export" class="flex items-center space-x-2 px-3 border border-green-400 rounded-md bg-white dark:bg-gray-700 dark:text-green-400 text-green-500 dark:hover:text-green-200 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-green-200 dark:hover:bg-green-600 focus:outline-none"><span>{{ __('Export') }}</span>
        ...
        <div class="rounded-lg shadow-lg bg-white dark:bg-gray-800 max-w-screen overflow-x-scroll">
            <div class="rounded-lg @unless($this->hidePagination) rounded-b-none @endif">
                <div class="table align-middle min-w-full">
                    ...
                    <div class="table-row divide-x divide-gray-200 dark:divide-gray-600">
                        @foreach($this->columns as $index => $column)
                            @if($hideable === 'inline')
                    ...
                    <div class="table-row divide-x divide-cool-gray-300 dark:divide-cool-gray-600 bg-cool-gray-200 dark:bg-cool-gray-700">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                @if($hideable === 'inline')
                    ...
                    @forelse($this->results as $result)
                        <div class="table-row p-1 divide-x divide-gray-100 dark:divide-gray-600 {{ isset($result->checkbox_attribute) && in_array($result->checkbox_attribute, $selected) ? 'bg-orange-100' : ($loop->even ? 'bg-gray-100 dark:bg-gray-650' : 'bg-gray-50 dark:bg-gray-700') }}">
                            @foreach($this->columns as $column)
                                ...
                                @else
                                    <div class="px-6 py-2 whitespace-no-wrap text-sm leading-5 text-gray-900 dark:text-gray-200 table-cell @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @else text-left @endif">
                                        <div id="{{ explode('.',$column['name'])[0] }}">{!! $result->{$column['name']} !!}</div>
                                    </div>
                                @endif
                            @endforeach
                        </div>
                    ...
                    <div>
                        <p class="pt-3 text-lg text-center text-red-600 dark:text-red-400">
                            {{ __("No devices found with the selected filters") }}
                        </p>  
                    </div>
                    @endforelse
            ...
            <div class="rounded-lg rounded-t-none max-w-screen border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                <div class="p-2 sm:flex items-center justify-between">
                    {{-- check if there is any data --}}
                    @if(count($this->results))
                        <div class="my-2 sm:my-0 flex items-center">
                            <select name="perPage" class="mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5 rounded dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300" wire:model="perPage">
