...
<div
    ...
    @else
        <button wire:click.prefetch="sort('{{ $index }}')"
            class="w-full h-full px-6 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 dark:text-gray-300 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex items-center focus:outline-none">
            <span class="inline flex-grow @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @endif"">{{ str_replace('_', ' ', $column['label']) }}</span>
            <span class="inline text-xs text-blue-400">
                @if($sort === $index)
                @if($direction)
                <x-icons.chevron-up class="h-6 w-6 text-blue-600 dark:text-blue-400 stroke-current" />
                @else
                <x-icons.chevron-down class="h-6 w-6 text-blue-600 dark:text-blue-400 stroke-current" />
                @endif
                @endif
            </span>
        </button>
